...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%Local%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% classify elements based on size inscribed sphere as the characteristic length

AtoV=3/2*sum(Fscale([1,Nfp+1,2*Nfp+1,3*Nfp+1],:));
diameter=6./AtoV;
dtscale=min(diameter)

%%
BCType = zeros(K,4);

%%  Setup_BCmap2DRandT
BCType = CorrectBCTable3D2(EToV,BCType,tri_slip,Slip);
BCType = CorrectBCTable3D2(EToV,BCType,tri_CNRBC,CNRBC);
BCType = CorrectBCTable3D2(EToV,BCType,tri_impedance,Impedance);
for i=1:num_bc
eval(['BCType = CorrectBCTable3D2(EToV,BCType,tri_impedance',num2str(i),',Impedance',num2str(i),');']);
end

for i=1:Tnum_bc
eval(['BCType = CorrectBCTable3D2(EToV,BCType,tri_trans',num2str(i),',Trans',num2str(i),');']);
end
